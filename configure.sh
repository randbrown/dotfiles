...
echo ''
read -p "Do you want to use randbrown's dotfiles? y/n" -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    echo ''
	echo "Now pulling down randbrown dotfiles..."
    ...
    if [[ $? -eq 0 ]]
    then
        echo "Successfully configured your environment with randbrown's dotfiles..."
    else
        echo "randbrown's dotfiles were not applied successfully..." >&2
fi
else 
	echo ''
    echo "You chose not to apply randbrown's dotfiles. You will need to configure your environment manually..."
	echo ''
